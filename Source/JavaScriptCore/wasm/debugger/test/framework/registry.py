"""
Test registry for the WebAssembly Debugger Test Framework
"""
...
class TestRegistry:
    ...
    def get_all_test_classes(self) -> Dict[str, Type[BaseTestCase]]:
        """Get all registered test classes"""
        return self.test_classes.copy()

    def create_test_instance(self, name: str, build_config: str = None) -> BaseTestCase:
        ...
        return test_class(build_config=build_config)

    def unregister_test(self, name: str):
        """Remove a test from the registry"""
        if name in self.test_classes:
            del self.test_classes[name]
        if name in self.test_order:
            self.test_order.remove(name)
