"""
WebKit environment management for the WebAssembly Debugger Test Framework
"""
...
class WebKitEnvironment:
    ...
    def __init__(self, script_path: Path, build_config: str = None):
        ...
        # Path: Source/JavaScriptCore/wasm/debugger/test/test-wasm-debugger.py
        # We need to go up 6 levels to get to the WebKit root
        # test -> debugger -> wasm -> JavaScriptCore -> Source -> OpenSource -> WebKit root
        self.webkit_root = (
            script_path.resolve().parent.parent.parent.parent.parent.parent
        )
    ...
    def _find_jsc_path(self):
        ...
        if self.build_config:
            ...
            if jsc_path.exists():
                Logger.info(f"Found JSC at: {jsc_path} (using --{self.build_config.lower()} option)")
                ...
                Logger.info(f"  Run: Tools/Scripts/build-webkit --{self.build_config.lower()}")
    ...
    def validate(self) -> bool:
        ...
        if self.lldb_path == "lldb":
            ...
            if Path(self.lldb_path).exists():
                ...
                Logger.warning(f"Custom LLDB not found at {self.lldb_path}, falling back to system LLDB")
                self.lldb_path = "lldb"
    ...
    def print_info(self):
        ...
        Logger.info(f"WebKit root: {self.webkit_root}")
        Logger.info(f"VM path: {self.vm_path}")
        Logger.info(f"JSC path: {self.jsc_path}")
        Logger.info(f"LLDB path: {self.lldb_path}")