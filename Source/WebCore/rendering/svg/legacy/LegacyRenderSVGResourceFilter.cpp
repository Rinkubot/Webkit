...
std::optional<std::tuple<Ref<Filter>, FloatRect>> LegacyRenderSVGResourceFilter::createFilter(RenderElement& renderer, const FloatRect& targetBoundingBox, GraphicsContext& context)
{
    ...
    auto filter = SVGFilter::create(filterElement, preferredFilterModes, filterScale, filterRegion, targetBoundingBox, context, RenderingResourceIdentifier::generate());
    if (!filter)
        return std::nullopt;
